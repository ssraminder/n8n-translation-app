...
export async function POST(req: NextRequest) {
  ...
  const { data: quote, error: qErr } = await client.from('quote_submissions').update({ name: client_name, email: client_email, status: 'submitted' }).eq('quote_id', quote_id).select('quote_id').single()
